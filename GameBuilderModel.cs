...
namespace GameBuilderEditor
{
    public sealed class GameBuilderModel : ScriptableObject
    {
        ...
        {
            [Tooltip("If checked, this settings will not be for a build, but for grouping other builds under itself")]
            ...
            [Tooltip(
                ...
                "{2}: build number (starts from 1, increases by each and not saved to git)")]
            public string buildPath = "Builds/{0}/Game{1}";

            [Tooltip("Version number of the build. \n" +
                     "{0}: build version value from Player Settings\n" +
                     "{1}: build number (starts from 1, increases by each and not saved to git)")]
            public string versionNumber = "{0}b{1}";
            ...
            [SerializeReference]
            public IBuildPostProcessor[] postProcessors = Array.Empty<IBuildPostProcessor>();

            private readonly static Dictionary<BuildOptions, string> s_infoCache = new();

            public string Info => GetCachedInfoString(buildOptions);

            private static string GetCachedInfoString(BuildOptions buildOptions)
            {
                if (s_infoCache.TryGetValue(buildOptions, out var str))
                {
                    return str;
                }
                ...
                str = sb.ToString();
                s_infoCache[buildOptions] = str;
                return str;
            }

            internal BuildSettings Copy()
            {
                return new()
                {
                    isGroupHeader = isGroupHeader,
                    buildingPlatform = buildingPlatform,
                    scenes = scenes.AsEnumerable().ToArray(),
                    scriptingDefines = scriptingDefines.AsEnumerable().ToArray(),
                    buildPath = buildPath,
                    versionNumber = versionNumber,
                    label = label,
                    buildOptions = buildOptions,
                    postProcessors = postProcessors.Select(p => p.Copy()).ToArray()
                };
            }
        }
    }
